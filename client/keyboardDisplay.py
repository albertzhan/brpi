...
def keyboard(surface, height,x,y,mx,my,mb):
    ...
    for i in range(4):
        for j in range(7):
            ...
            surface.blit(textSurface,(30-textSurface.get_width()//2+j*60,height//2-textSurface.get_height()//2+i*height))
    for i in range(4):
        for j in range(3):
            n = numpad[i*3+j]
            if n in ["*","#"]:
            ...
            surface.blit(textSurface,(571-textSurface.get_width()//2+j*60,height//2-textSurface.get_height()//2+i*height))
